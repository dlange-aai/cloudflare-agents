...
You need an [AssemblyAI API key](https://www.assemblyai.com/app/api-keys) for STT and an [OpenAI API key](https://platform.openai.com/api-keys) for the LLM. TTS runs on the Workers AI binding — run `wrangler login` once so local dev can reach it.
